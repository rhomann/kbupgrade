...
def read_keycode_enumeration(file):
  ...
  while True:
    ...
    if re.match("KEY_",name): num_of_keys+=1
    else:                     num_of_mods+=1

    assignment=re.split("=",line)
    if len(assignment) > 1:
      assignment=re.split("[, \t]",assignment[1],1)
      if assignment[0]:
        try:
          code=int(assignment[0],0)
        except ValueError as err:
          error('Could not parse assigned value: '+str(err))
...
def keyname_to_binary(valid_keycodes,name):
  if name == 'KEY_trash': return 0
  assert name in valid_keycodes, "invalid name "+name
  return valid_keycodes[name]

def write_binary_keymap(file,options):
  check_name(options)
  file.write(options.keymap_name)
  for i in range(0,options.kbdef.max_name_length-len(options.keymap_name)):
    file.write("\0")
  sparse_matrix=options.kbdef.get_sparse_matrix()
  bits=bytearray(0)
  for x in sparse_matrix:
    bits.append(keyname_to_binary(options.kbdef.valid_keycodes,x.name))
  file.write(bits)
...
def main():
  ...
  new_file(keymap_binfile,write_binary_keymap,options)
