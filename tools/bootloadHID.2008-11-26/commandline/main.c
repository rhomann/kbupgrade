...
static char dataBufferGlobal[65536 + 256];    /* buffer for file data */
...
char    *file;
    ...
    startAddress = sizeof(dataBufferGlobal);
    endAddress = 0;
    memset(dataBufferGlobal, -1, sizeof(dataBufferGlobal));
    if(parseIntelHex(file, dataBufferGlobal, &startAddress, &endAddress))
        return 1;
    if(startAddress >= endAddress){
        fprintf(stderr, "No data in input file, exiting.\n");
        return 0;
    }
    if(uploadData(dataBufferGlobal, startAddress, endAddress))
        return 1;
    return 0;
}
